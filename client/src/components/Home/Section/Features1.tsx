   ...
   return (
      <div className="flex items-center  m-2 mt-4 space-x-4 rounded-xl cursor-pointer hover:bg-gray-100 hover:scale-105 transition transform group">
         <div className="relative h-60 w-60">
            ...
            <a
               href="/"
               className="absolute top-1/2 left-1/2 h-14 w-14 bg-white flex justify-center items-center rounded-full -translate-x-1/2 -translate-y-1/2 scale-0 group-hover:scale-100 transition-all duration-700"
            >
         ...
         <div className="">
            ...
            <h3 className="text-3xl pt-4 font-normal text-gray-500"> {des}</h3>
         </div>
      </div>
   );
};
   ...
   return (
      <div className="padding-container py-12">
         <div className="m-auto w-[90%]">
            <h3 className="bold-40 font-extrabold text-center">
               Features Destination
            </h3>
            <p className="regular-18 text-center">
               Lorem ipsum dolor sit amet consectetur adipisicing elit. Ab totam
               nobis soluta tempora
            </p>
         </div>
         <div className="grid items-center gap-4 grid-cols-1 sm:grid-flow-cols-2 lg:grid-cols-3 pt-12">
            {DESTINATION.map(({ title, url, des }) => {
               return <FeaturesItem title={title} url={url} des={des} />;
            })}
         </div>
      </div>
   );
